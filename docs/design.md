...
5. 자전거 종류는 **일반(1인)**, **2인용**, **어린이** 3종으로 구분한다. 어린이 자전거는 만 13세 미만(초등학생 이하)만 대여 가능하며, 연령은 `User.user_birth_date` 기준 대여 시점에 실시간으로 계산한다.
...
| 컬럼명 | 타입 | 제약 | 설명 |
|--------|------|------|------|
| `bicycleType_id` | INT | PK, AUTO_INCREMENT | 종류 ID |
...
> 자전거 정비는 **외부 업체(외주)에 위탁**하여 진행한다. 본 테이블은 "정비를 외주에 맡겼다"는 사실과 완료 여부만을 기록하며, 외주 업체의 작업자·정비 방법은 관리하지 않는다.
...
자전거 단말기가 주기적으로 `Bicycle.bicycle_gps_latitude / bicycle_gps_longitude / bicycle_gps_updated_at`을 갱신하며, 시흥시 경계 이탈이 감지되면 자동으로 회수 프로세스가 시작된다.

```
[GPS 갱신] Bicycle.bicycle_gps_latitude, bicycle_gps_longitude, bicycle_gps_updated_at 업데이트
  └─▶ 시흥시 경계 이탈 여부 판정
      └─▶ 이탈 감지 시
...
- `TIMESTAMPDIFF(YEAR, User.user_birth_date, CURRENT_DATE) < 13`(만 13세 미만)이면 `BicycleType.bicycleType_name = '어린이'` 자전거만 대여 가능
- 어린이 자전거의 `max_passenger = 1`
- 연령 판정은 대여 시점에 `user_birth_date`로 실시간 계산한다. `is_minor` 컬럼은 `user_id → user_birth_date → is_minor` 이행적 함수 종속에 해당하므로 3NF 준수를 위해 제거하였다.
...
현재 스키마에서 의도적 비정규화는 적용되지 않는다. 모든 컬럼은 해당 엔터티의 기본키에 직접 종속된다.
