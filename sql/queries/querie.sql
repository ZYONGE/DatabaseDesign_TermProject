...
SELECT
    ...
    SUM(CASE WHEN bt.type_name = '일반'   THEN 1 ELSE 0 END)           AS normal_count,
    SUM(CASE WHEN bt.type_name = '어린이' THEN 1 ELSE 0 END)           AS child_count,
    SUM(CASE WHEN bt.type_name = '2인용'  THEN 1 ELSE 0 END)           AS tandem_count
FROM Station s
JOIN Region rg ON s.region_id = rg.region_id
LEFT JOIN Bicycle b  ON b.current_station_id = s.station_id
                     AND b.bike_status = '정상'
LEFT JOIN BicycleType bt ON b.type_id = bt.type_id
...
JOIN BicycleType bt ON b.type_id          = bt.type_id
...
SELECT
    ...
    r.rental_date,
    r.start_time,
    DATEDIFF(CURDATE(), r.rental_date)                               AS overdue_days,
    DATEDIFF(CURDATE(), r.rental_date) * 10                         AS penalty_days,
    CURDATE()                                                        AS ban_start,
    DATE_ADD(CURDATE(),
        INTERVAL DATEDIFF(CURDATE(), r.rental_date) * 10 DAY)       AS ban_end
FROM Rental r
JOIN User u ON r.user_id = u.user_id
WHERE r.rental_status = '대여중'
...
SELECT
    ...
    -- 조건 4: 자전거 상태
    b.bike_status                                                               AS cond4_bike_status,
    -- 조건 5: 어린이 자전거 연령 제한
    CASE
        WHEN TIMESTAMPDIFF(YEAR, u.birth_date, CURDATE()) < 13
             AND bt.type_name != '어린이'  THEN '불가: 만 13세 미만은 어린이 자전거만 가능'
        WHEN TIMESTAMPDIFF(YEAR, u.birth_date, CURDATE()) >= 13
             AND bt.type_name  = '어린이' THEN '불가: 어린이 자전거는 만 13세 미만 전용'
        ELSE '조건 충족'
    ...
    CASE
        ...
        WHEN (SELECT COUNT(*) FROM Penalty p WHERE p.user_id = u.user_id AND p.ban_start <= CURDATE() AND (p.ban_end IS NULL OR p.ban_end >= CURDATE())) > 0
            THEN '대여 불가: 패널티 적용 중'
        WHEN b.bike_status != '정상'
            THEN CONCAT('대여 불가: 자전거 상태 [', b.bike_status, ']')
        WHEN TIMESTAMPDIFF(YEAR, u.birth_date, CURDATE()) < 13 AND bt.type_name != '어린이'
...
SELECT
    rg.region_name,
    DATE_FORMAT(r.rental_date, '%Y-%m')                             AS ym,
...
SELECT
    ...
    SUM(CASE WHEN b.bike_status = '정상'   THEN 1 ELSE 0 END)      AS available,
    SUM(CASE WHEN b.bike_status = '대여중' THEN 1 ELSE 0 END)      AS in_use,
    SUM(CASE WHEN b.bike_status = '정비중' THEN 1 ELSE 0 END)      AS in_maintenance,
    SUM(CASE WHEN b.bike_status = '회수중' THEN 1 ELSE 0 END)      AS retrieving,
    SUM(CASE WHEN b.bike_status = '분실'   THEN 1 ELSE 0 END)      AS lost,
    ROUND(
        SUM(CASE WHEN b.bike_status = '대여중' THEN 1 ELSE 0 END)
        / NULLIF(COUNT(b.bicycle_id), 0) * 100, 1
    )                                                                AS utilization_pct
FROM BicycleType bt
LEFT JOIN Bicycle b ON bt.type_id = b.type_id
GROUP BY bt.type_id, bt.type_name, bt.max_passenger
ORDER BY bt.type_id;
...
SELECT
    ...
    i.incident_type,
    i.incident_status,
    COALESCE(u.name, '관리자 직접 접수')                             AS reporter,
    b.bicycle_id,
    bt.type_name                                                     AS bike_type,
    i.description,
    i.reported_at,
...
JOIN      BicycleType bt ON b.type_id          = bt.type_id
LEFT JOIN AdminStaff a  ON i.assigned_staff_id = a.staff_id
...
JOIN  Station    ts  ON rv.target_station_id = ts.station_id
...
SELECT
    ...
    m.started_at,
    m.ended_at,
    COALESCE(rs.station_name, '미확정')                             AS return_station,
...
JOIN  BicycleType  bt ON b.type_id           = bt.type_id
LEFT JOIN Station  rs ON m.return_station_id = rs.station_id
