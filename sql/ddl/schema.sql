...
CREATE TABLE BicycleType (
    ...
    description      MEDIUMTEXT            DEFAULT NULL COMMENT '종류 설명',
...
CREATE TABLE Station (
    ...
    contactnumber     VARCHAR(20)            DEFAULT NULL  COMMENT '연락처',
    ...
    totaldocks        INT                    DEFAULT 0     COMMENT '총 거치대 수',
    region_id         INT           NOT NULL               COMMENT '소속 지역 (FK)',
...
CREATE TABLE Bicycle (
    ...
    type_id                 INT           NOT NULL               COMMENT '자전거 종류 (FK)',
    station_id              INT                    DEFAULT NULL   COMMENT '현재 대여소 (FK)',
    PRIMARY KEY (bicycle_id),
    CONSTRAINT fk_bicycle_type    FOREIGN KEY (type_id)    REFERENCES BicycleType(bicycleType_id),
    CONSTRAINT fk_bicycle_station FOREIGN KEY (station_id) REFERENCES Station(station_id)
...
CREATE TABLE AdminStaff (
    ...
    station_id       INT                  DEFAULT NULL  COMMENT '담당 대여소 (FK)',
...
CREATE TABLE Rental (
    ...
    start_time       DATETIME NOT NULL               COMMENT '대여 시작일시',
    end_time         DATETIME          DEFAULT NULL  COMMENT '반납일시',
    rental_status    ENUM('대여중','반납','미반납')
                              NOT NULL DEFAULT '대여중',
    start_station_id INT      NOT NULL               COMMENT '시작 대여소 (FK)',
    end_station_id   INT               DEFAULT NULL  COMMENT '반납 대여소 (FK)',
    bicycle_id       INT      NOT NULL               COMMENT '대여 자전거 (FK)',
    user_id          INT      NOT NULL               COMMENT '이용 사용자 (FK)',
...
CREATE TABLE IncidentReport (
    ...
    reported_at     DATETIME          DEFAULT NOW() COMMENT '신고일시',
    incident_status ENUM('접수','처리중','완료')
                             NOT NULL DEFAULT '접수',
    resolved_at     DATETIME          DEFAULT NULL  COMMENT '처리 완료일시',
    bicycle_id      INT               DEFAULT NULL  COMMENT '신고 자전거 (FK)',
    user_id         INT               DEFAULT NULL  COMMENT '신고자 (FK)',
    staff_id        INT      NOT NULL               COMMENT '담당 관리자 (FK)',
    ...
    CONSTRAINT fk_incident_staff   FOREIGN KEY (staff_id)   REFERENCES AdminStaff(staff_id)
...
CREATE TABLE Maintenance (
    ...
    description        MEDIUMTEXT            DEFAULT NULL  COMMENT '정비 의뢰 내용',
    started_at         DATETIME     NOT NULL               COMMENT '정비 의뢰일시',
    ended_at           DATETIME              DEFAULT NULL  COMMENT '정비 완료일시',
    maintenance_status ENUM('진행중','완료')
                                    NOT NULL DEFAULT '진행중',
    bicycle_id         INT          NOT NULL               COMMENT '정비 자전거 (FK)',
    incident_id        INT                   DEFAULT NULL  COMMENT '연계 신고 (FK)',
...
CREATE TABLE Retrieve (
    ...
    completed_at       DATETIME               DEFAULT NULL  COMMENT '반납 완료일시',
    retrieve_reason    ENUM('방치','미반납','구역이탈','기타')
                                    NOT NULL               COMMENT '회수 사유',
    retrieve_status    ENUM('진행중','완료')
                                    NOT NULL DEFAULT '진행중',
    bicycle_id         INT                    DEFAULT NULL  COMMENT '회수 자전거 (FK)',
    staff_id           INT                    DEFAULT NULL  COMMENT '담당 관리자 (FK)',
    incident_id        INT                    DEFAULT NULL  COMMENT '연계 신고 (FK)',
    station_id         INT           NOT NULL               COMMENT '반납 목표 대여소 (FK)',
    ...
    CONSTRAINT fk_retrieve_station  FOREIGN KEY (station_id)  REFERENCES Station(station_id)
...
CREATE TABLE Penalty (
    ...
    ban_end      DATE     NOT NULL               COMMENT '금지 종료일',
    user_id      INT      NOT NULL               COMMENT '대상 사용자 (FK)',
    rental_id    INT      NOT NULL UNIQUE        COMMENT '미반납 대여 건 (FK)',
...
CREATE TABLE Allocation (
    ...
    note          MEDIUMTEXT            DEFAULT NULL  COMMENT '비고',
    station_id    INT                   DEFAULT NULL  COMMENT '배치 대여소 (FK)',
    staff_id      INT                   DEFAULT NULL  COMMENT '배치 담당자 (FK)',
    bicycle_id    INT                   DEFAULT NULL  COMMENT '배치 자전거 (FK)',
    PRIMARY KEY (allocation_id),
    CONSTRAINT fk_allocation_station FOREIGN KEY (station_id) REFERENCES Station(station_id),
    CONSTRAINT fk_allocation_staff   FOREIGN KEY (staff_id)   REFERENCES AdminStaff(staff_id),
